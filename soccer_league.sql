-- This SQL script creates a database schema for a soccer league. It defines several tables to store information about teams, players, referees, seasons, matches, goals, lineups, and results.
...
CREATE TABLE Matches (
    ...
    referee_id INT, -- ID of the referee
...
CREATE TABLE Goals (
    ...
    player_id INT, -- ID of the player who scored the goal
    match_id INT, -- ID of the match the goal was scored in
...
-- The "Lineups" table stores information about each player's lineup in a match, including the ID of the player, the ID of the match, and the ID of the team. The player ID, match ID, and team ID are foreign keys referencing the "Players", "Matches", and "Teams" tables respectively.
...
-- The "Results" table stores information about each match result, including the ID of the team, the ID of the match, and the result (wins, loses, or draw). The team ID and match ID are foreign keys referencing the "Teams" and "Matches" tables respectively.
CREATE TABLE Results (
    team_id INT, -- ID of the team
    match_id INT, -- ID of the match
    result VARCHAR(10) NOT NULL, -- Result of the match (wins, loses, or draw)
    FOREIGN KEY (team_id) REFERENCES Teams(team_id) ON DELETE SET NULL, -- Establish a relationship with the "Teams" table
    FOREIGN KEY (match_id) REFERENCES Matches(match_id) ON DELETE SET NULL -- Establish a relationship with the "Matches" table
);
...
-- Insert sample data into the "Lineups" table
INSERT INTO Lineups 
